...
**实测特性（重要，勿踩坑）**：API 返回的评论几乎全部带正文（实测 179/179），它给出的是近期窗口内**有正文评论的近完整采样**，而非全量随机采样。

- **适合**：主题分析、问题发现、原文下钻。全量数据里带正文的通常只有一成，API 相当于替你筛过一道。
- **不适合**：差评率、均分等比例指标。愿意写文字的用户负面偏好更强，实测有正文子集的差评率是全量的约 **1.76 倍**（某应用实测：全量 7.28%，有正文 12.81%）。同一版本用 API 口径算出 8.7%，全量口径只有 4.63%——据此下结论会严重误判。

比例指标一律以 CSV / GCS 全量口径为准，API 只用于内容与早期信号。
